...
epsilon = 0.1
###################
state_action_space = StateActionSpace(([-1.2, -0.07], [0.6, 0.07]), precise, [0, 1])
...
for i_episode in range(50):
    ...
    discret_state = state_action_space._m_state_continuous_to_discret(observation)
    discret_state_bar = discret_state

    action = learning_agent._m_GreedyPolicy(discret_state)

    phi = state_action_space._m_discret_state_to_feature(discret_state, action)
    ...
    for t in range(120):
        ...
        while set(discret_state) == set(discret_state_bar):
            ...
            discret_state_bar = state_action_space._m_state_continuous_to_discret(observation_bar)
            same_action_count += 1
            if same_action_count > 100:
                print "fuck"
                break
        action_bar = learning_agent._m_GreedyPolicy(discret_state_bar)
        phi_bar = state_action_space._m_discret_state_to_feature(discret_state_bar, action_bar)
...
#         # print np.dot(Qfunc_previous - Qfunc, Qfunc_previous - Qfunc)
    ...
    Qfunc_difference.append(np.dot(Qfunc_previous - learning_agent.theta, Qfunc_previous - learning_agent.theta))
    if i_episode % 100 == 0:
