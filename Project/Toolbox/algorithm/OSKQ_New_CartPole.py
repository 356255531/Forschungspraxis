...
class OSKQ_New_CartPole(object):
    ...
    def __init__(self,
        ...
        super(OSKQ_New_CartPole, self).__init__()
        self.mu_1 = mu_1
        self.mu_2 = mu_2
        self.alpha = alpha
        self.gamma = gamma
    ...
    def dict_update(
        ...
        if already_in_dict == 0:
            self.dict = np.concatenate(
                (self.dict, feature_vector.reshape((1, 5))),
                axis=0
            )
    ...
    def gaussian_kernel(self, feature_vector):
        ...
        minuser = np.dot(one_vector, feature_vector.reshape((1, 5)))
