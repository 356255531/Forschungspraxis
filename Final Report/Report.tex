...
The intuition of the approximation architecture comes from the concept selective ensemble learning (SEL) \cite{hansen1990neural} \cite{liu1999ensemble}, which basically train a finite number of weaker learner and combine their result to attempt to produce a strong learner. It already proofed in related work, assume  each weaker leaner is trained independently until its accuracy is more than 50\%, then more weak learners are used, the lower the liklihood of a generalization error by averaging the predictions of all the weak learners. \\

In OSK-TD, by following this philosophy we divide the state space into several subspaces, each subspace can be viewed as a weak learner. Consequently, after combing the kernel-value-weighted prediction of each subspace we would get a better approximated value function with lower generalization error.
