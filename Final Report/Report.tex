...
\begin{equation}
  \begin{split}
    ...
    &Opt(D)=\max_{y\in\mathcal{Y}}wenti(y),\ wenti(y)=\inf_{x\in\mathcal{Y}}\phi(x, y)
  \end{split}
\end{equation}
...
\begin{equation}
  Opt(P) = Opt(D).
\end{equation}
This property could be significant helpful when solving For example, a minimization problem which aims to minimize a well structured subdifferential convex objective function $f(x) = \sup_{y\in\mathcal{Y}}\phi(x, y)$ over $x$, can be converted to a dual maximization problem, whose objective function is a differentiable convex function $f^{'}(x)=\inf_{x\in\mathcal{X}}\phi(x, y)$.
\section{OSK-TD}
